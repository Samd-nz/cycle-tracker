...
export default function App() {
  ...
  const sl   = {fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2.8,textTransform:"uppercase",color:T.color,opacity:.8,marginBottom:12,display:"block"};
  const inp  = (c=T.color) => ({width:"100%",padding:"10px 13px",borderRadius:10,border:`1px solid ${c}44`,background:"#ffffff0D",color:T.textColor,fontFamily:"'Raleway',sans-serif",fontSize:13,outline:"none",boxSizing:"border-box"});
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:T.bg,fontFamily:"'Cormorant Garamond',Georgia,serif",color:T.textColor,maxWidth:430,margin:"0 auto",position:"relative",transition:"background 1s"}}>
        ...
        <div style={{position:"relative",zIndex:1,padding:"30px 22px 14px",borderBottom:`1px solid ${T.color}28`}}>
          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:3.5,textTransform:"uppercase",color:T.color,opacity:.85}}>{fmtDisplay(new Date())}</p>
          ...
          {cur&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:10,color:T.color,letterSpacing:2.5,textTransform:"uppercase",marginTop:5,opacity:.8}}>{cur.tagline}</p>}
        </div>
        ...
        {view!=="phase"&&(
          <div style={{position:"relative",zIndex:1,display:"flex",borderBottom:`1px solid ${T.color}28`}}>
            {[["home","Today"],["log","Journal"],["stats","Stats"]].map(([v,l])=>(
              <button key={v} onClick={()=>setView(v)} style={{flex:1,padding:"11px",border:"none",background:"transparent",fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:3,textTransform:"uppercase",color:view===v?T.color:T.mutedColor,borderBottom:view===v?`1.5px solid ${T.color}`:"1.5px solid transparent",cursor:"pointer",transition:"all .3s"}}>{l}</button>
            ))}
          </div>
        )}
              ...
              {!cycleStartDate?(
                ...
                <div style={{...card(),display:"flex",gap:16,alignItems:"flex-start"}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,minWidth:78}}>
                    ...
                    <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:1.5,textTransform:"uppercase",color:T.color,textAlign:"center",opacity:.85,lineHeight:1.4}}>{moon.name}</p>
                  </div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:3}}>
                      <span style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.8}}>Day</span>
                      <span style={{fontSize:26,color:T.color,fontWeight:300,lineHeight:1}}>{doc}</span>
                      <span style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.mutedColor,opacity:.8}}>of ~28</span>
                    </div>
                  ...
                  <div style={card()}>
                    ...
                    {[["✦ Energy",guide.energy],["✦ Mood",guide.mood],["✦ Body",guide.body]].map(([label,val])=>(
                      <div key={label} style={{display:"flex",gap:12,marginBottom:9,alignItems:"flex-start"}}>
                        <span style={{fontFamily:"'Raleway',sans-serif",fontSize:10,color:T.color,minWidth:62,opacity:.85,marginTop:2}}>{label}</span>
                    ...
                    <div style={{marginTop:14,padding:"12px 14px",background:T.color+"18",borderRadius:10,borderLeft:`2px solid ${T.color}66`}}>
                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.85,marginBottom:5}}>Today's ritual</p>
                ...
                <div style={card()}>
                  ...
                  <button onClick={()=>{setActivePhase(curKey);setPhaseTab("care");setView("phase")}} style={{background:"none",border:`1px solid ${T.color}55`,borderRadius:100,padding:"7px 18px",color:T.color,fontFamily:"'Raleway',sans-serif",fontSize:10,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>Full phase guide →</button>
                </div>
                ...
                {nextPeriod&&(
                  <div style={{...card(),display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <div style={{textAlign:"center"}}>
                      ...
                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.color,letterSpacing:1.5,textTransform:"uppercase",opacity:.8}}>days</p>
                    </div>
                  </div>
                )}
                ...
                <div style={card()}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {Object.entries(PHASES).map(([k,p])=>(
                      <button key={k} onClick={()=>{setActivePhase(k);setPhaseTab("care");setView("phase")}} style={{background:curKey===k?p.color+"28":"transparent",color:p.color,border:`1px solid ${p.color}55`,borderRadius:100,padding:"5px 14px",fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",cursor:"pointer"}}>
                        {p.emoji} {p.name}
                      </button>
                    ))}
                  </div>
                ...
                <div style={card()}>
                  ...
                  {ageView&&(
                    <div style={{background:T.color+"10",borderRadius:10,padding:"14px",marginTop:4,border:`1px solid ${T.color}28`}}>
                      ...
                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:10,color:T.color,opacity:.55,marginTop:10}}>Always speak with your GP about significant cycle changes.</p>
                    </div>
                  )}
                </div>
              </>)}
            ...
            <div style={{padding:"18px 20px 100px"}} className="fi">
              ...
              {cycleGroups.length===0?(
                <div style={{...card(),textAlign:"center",padding:"30px 20px"}}>
                  <p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.mutedColor,lineHeight:1.8}}>No entries yet.<br/>Add your first entry above to begin your journal.</p>
                </div>
              ):(
                  ...
                  return (
                    <div key={key} style={{marginBottom:10}}>
                      ...
                      <button onClick={()=>toggleGroup(key)} style={{width:"100%",background:T.cardBg,borderRadius:isOpen?"16px 16px 0 0":16,padding:"16px 18px",border:`1px solid ${T.color}28`,borderBottom:isOpen?`1px solid ${T.color}18`:"none",cursor:"pointer",textAlign:"left",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                        <div style={{flex:1}}>
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2.5,textTransform:"uppercase",color:T.color,opacity:.8,marginBottom:6}}>
                            {group.isCurrent?"Current cycle":"Cycle"}
                          </p>
                          <p style={{fontSize:14,color:T.textColor,lineHeight:1.3,marginBottom:8}}>{label}</p>
                          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                            <div>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.65,marginBottom:2}}>Cycle length</p>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:13,color:group.cycleLength?T.textColor:T.mutedColor}}>
                                {group.cycleLength?`${group.cycleLength} days`:"Ongoing"}
                              </p>
                            </div>
                            <div>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.65,marginBottom:2}}>Days bleeding</p>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:13,color:group.daysBleeding?T.textColor:T.mutedColor}}>
                                {group.daysBleeding?`${group.daysBleeding} days`:"Not logged"}
                              </p>
                            </div>
                            <div>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.65,marginBottom:2}}>Entries</p>
                              <p style={{fontFamily:"'Raleway',sans-serif",fontSize:13,color:T.textColor}}>{group.events.length}</p>
                            </div>
                          </div>
                        </div>
                        <span style={{color:T.color,fontSize:16,opacity:.7,marginLeft:12,marginTop:2,transition:"transform .3s",display:"block",transform:isOpen?"rotate(180deg)":"none"}}>⌄</span>
                      </button>
                      ...
                      {isOpen&&(
                        <div style={{background:T.cardBg,borderRadius:"0 0 16px 16px",border:`1px solid ${T.color}28`,borderTop:"none",overflow:"hidden"}}>
                          {group.events.length===0?(
                            <p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.mutedColor,padding:"16px 18px"}}>No entries in this cycle.</p>
                          ):(
                                ...
                                <div key={ev.id} style={{padding:"12px 18px",borderTop:`1px solid ${T.color}14`}}>
                                  {isEditing?(
                                    <div style={{background:T.color+"0C",borderRadius:12,padding:"14px"}}>
                                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.8,marginBottom:10}}>Edit entry</p>
                                      ...
                                      <div style={{marginBottom:14}}>
                                        <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.mutedColor,marginBottom:5,letterSpacing:1}}>Notes</p>
                                    ...
                                    <>
                                      <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                                        <span style={{fontSize:18,lineHeight:1,marginTop:2}}>{et.emoji}</span>
                                        <div style={{flex:1}}>
                                          ...
                                          {ev.note&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.mutedColor,marginTop:6,fontStyle:"italic",lineHeight:1.5,borderLeft:`2px solid ${T.color}44`,paddingLeft:8}}>{ev.note}</p>}
                                        </div>
                                        <div style={{display:"flex",gap:6,flexShrink:0}}>
                                          <button onClick={()=>startEdit(ev)} style={{background:"none",border:`1px solid ${T.color}44`,borderRadius:100,padding:"4px 11px",color:T.color,fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",cursor:"pointer"}}>Edit</button>
                                          <button onClick={()=>deleteEvent(ev.id)} style={{background:"none",border:`1px solid ${T.color}28`,borderRadius:100,padding:"4px 9px",color:T.mutedColor,fontFamily:"'Raleway',sans-serif",fontSize:9,cursor:"pointer"}}>✕</button>
                                        </div>
                                      </div>
                                    </>
                                  )}
          ...
          {view==="stats"&&(
            <div style={{padding:"18px 20px 100px"}} className="fi">
              ...
              <div style={card()}>
                ...
                <div style={{display:"flex",gap:7}}>
                  {[["all","All time"],["1y","Past year"],["6m","Past 6 months"]].map(([v,l])=>(
                    <button key={v} onClick={()=>setStatsRange(v)} style={{flex:1,padding:"8px 4px",border:`1px solid ${statsRange===v?T.color:T.color+"44"}`,borderRadius:8,fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:1,textTransform:"uppercase",cursor:"pointer",background:statsRange===v?T.color+"22":"transparent",color:statsRange===v?T.color:T.mutedColor,transition:"all .2s"}}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              {filteredGroups.length===0?(
                <div style={{...card(),textAlign:"center",padding:"30px 20px"}}>
                  <p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.mutedColor,lineHeight:1.8}}>No cycle data for this period yet.<br/>Log your period to begin building stats.</p>
                </div>
              ):(<>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    {[
                      ...
                      <div key={label} style={{background:T.color+"0C",borderRadius:12,padding:"14px",border:`1px solid ${T.color}28`}}>
                        <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.8,marginBottom:8,lineHeight:1.4}}>{label}</p>
                        {value!==null?(
                          <>
                            <p style={{fontSize:28,color:T.color,fontWeight:300,lineHeight:1}}>{value}</p>
                            <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.mutedColor,marginTop:2}}>{unit}</p>
                          </>
                        ):(
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.mutedColor,marginTop:4}}>Not enough data</p>
                        )}
                        {sub&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.mutedColor,opacity:.7,marginTop:6,lineHeight:1.4}}>{sub}</p>}
                      </div>
                    ))}
                    ...
                    <div onClick={()=>setShowUnexpected(p=>!p)} style={{background:unexpectedEvents.length>0?T.color+"14":T.color+"0C",borderRadius:12,padding:"14px",border:`1px solid ${unexpectedEvents.length>0?T.color+"55":T.color+"28"}`,cursor:"pointer",gridColumn:unexpectedEvents.length>0?"span 1":"span 1"}}>
                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,letterSpacing:2,textTransform:"uppercase",color:T.color,opacity:.8,marginBottom:8,lineHeight:1.4}}>Unexpected bleeding</p>
                      <p style={{fontSize:28,color:unexpectedEvents.length>0?"#E0A060":T.color,fontWeight:300,lineHeight:1}}>{unexpectedEvents.length}</p>
                      <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.mutedColor,marginTop:2}}>events</p>
                      {unexpectedEvents.length>0&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.color,opacity:.7,marginTop:6}}>Tap to view ↓</p>}
                    </div>
                  </div>
                </div>
                  ...
                  <div style={card()}>
                    ...
                    {[...unexpectedEvents].sort((a,b)=>b.date.localeCompare(a.date)).map((ev,i,arr)=>(
                      <div key={ev.id} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:i<arr.length-1?`1px solid ${T.color}18`:"none",alignItems:"flex-start"}}>
                        ...
                        <div style={{flex:1}}>
                          <p style={{fontSize:14,color:T.textColor}}>{fmtFull(ev.date)}</p>
                          {ev.note&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:11,color:T.mutedColor,marginTop:4,fontStyle:"italic",lineHeight:1.5}}>{ev.note}</p>}
                        </div>
                      </div>
                    ))}
                    {unexpectedEvents.length>=3&&(
                      <div style={{marginTop:14,padding:"12px",background:"#E0A06010",borderRadius:10,border:"1px solid #E0A06033"}}>
                        <p style={{fontFamily:"'Raleway',sans-serif",fontSize:11,color:"#E0A060",lineHeight:1.7}}>Three or more unexpected bleeding events is worth discussing with your GP or gynaecologist.</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div style={card()}>
                  ...
                  {filteredGroups.map((g,i)=>(
                    <div key={g.cycleStart} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:i<filteredGroups.length-1?`1px solid ${T.color}18`:"none"}}>
                      <div>
                        <p style={{fontFamily:"'Raleway',sans-serif",fontSize:12,color:T.textColor,marginBottom:2}}>{fmtShort(g.cycleStart)}</p>
                        {g.isCurrent&&<p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:T.color,letterSpacing:1.5,textTransform:"uppercase",opacity:.8}}>Current</p>}
                      </div>
                      <div style={{display:"flex",gap:16,textAlign:"right"}}>
                        <div>
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,color:T.mutedColor,letterSpacing:1.5,textTransform:"uppercase",marginBottom:2}}>Length</p>
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:13,color:g.cycleLength?T.textColor:T.mutedColor}}>{g.cycleLength?`${g.cycleLength}d`:"–"}</p>
                        </div>
                        <div>
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:8,color:T.mutedColor,letterSpacing:1.5,textTransform:"uppercase",marginBottom:2}}>Bleeding</p>
                          <p style={{fontFamily:"'Raleway',sans-serif",fontSize:13,color:g.daysBleeding?T.textColor:T.mutedColor}}>{g.daysBleeding?`${g.daysBleeding}d`:"–"}</p>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div style={{background:disp.bg,minHeight:"100vh",transition:"background .6s"}}>
              <div style={{padding:"18px 20px 100px"}} className="fi">
                ...
                <div style={{textAlign:"center",marginBottom:22}}>
                  ...
                  <p style={{fontFamily:"'Raleway',sans-serif",fontSize:9,color:disp.color,letterSpacing:3,textTransform:"uppercase",marginTop:6,opacity:.9}}>{disp.tagline}</p>
                ...
                <div style={{background:disp.cardBg,borderRadius:16,padding:"18px",marginBottom:12,border:`1px solid ${disp.color}28`}}>
                  <span style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2.8,textTransform:"uppercase",color:disp.color,opacity:.8,marginBottom:12,display:"block"}}>What to expect</span>
                  {disp.symptoms.map((s,i)=>(
                    <div key={i} style={{display:"flex",gap:12,padding:"8px 0",borderBottom:i<disp.symptoms.length-1?`1px solid ${disp.color}18`:"none"}}>
                      <span style={{color:disp.color,fontSize:10,marginTop:3,opacity:.8}}>✦</span>
                ...
                {["care","food","supplements"].map(tab=>tab===phaseTab&&(
                  <div key={tab} style={{background:disp.cardBg,borderRadius:16,padding:"18px",marginBottom:12,border:`1px solid ${disp.color}28`}}>
                    <span style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2.8,textTransform:"uppercase",color:disp.color,opacity:.8,marginBottom:12,display:"block"}}>
                    ...
                    {(tab==="care"?disp.selfCare:tab==="food"?disp.foods:disp.supplements).map((item,i,arr)=>(
                      <div key={i} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:i<arr.length-1?`1px solid ${disp.color}18`:"none",alignItems:"flex-start"}}>
                        <span style={{color:disp.color,fontSize:10,marginTop:4,opacity:.8}}>✦</span>
                        <div>
                ...
                <div style={{background:disp.cardBg,borderRadius:16,padding:"18px",marginBottom:12,border:`1px solid ${disp.color}28`}}>
                  <span style={{fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:2.8,textTransform:"uppercase",color:disp.color,opacity:.8,marginBottom:12,display:"block"}}>Other phases</span>
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {Object.entries(PHASES).filter(([k])=>k!==dispKey).map(([k,p])=>(
                      <button key={k} onClick={()=>{setActivePhase(k);setPhaseTab("care")}} style={{background:"transparent",color:p.color,border:`1px solid ${p.color}55`,borderRadius:100,padding:"5px 14px",fontFamily:"'Raleway',sans-serif",fontSize:9,letterSpacing:1.5,textTransform:"uppercase",cursor:"pointer"}}>
                        {p.emoji} {p.name}
                      </button>
                    ))}
                  </div>
        ...
        {(view==="home"||view==="stats")&&(
          <button onClick={()=>setView("log")} style={{position:"fixed",bottom:26,right:20,width:52,height:52,borderRadius:"50%",background:T.color,color:T.bg,border:"none",fontSize:20,cursor:"pointer",boxShadow:`0 0 28px ${T.color}66`,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Cormorant Garamond',serif"}}>
            ✦
          </button>
        )}
      </div>
